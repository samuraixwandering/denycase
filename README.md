...
By default the principal is injected as `X-Denycase-Tenant` and `X-Denycase-Principal`. `Principal.Tenant` and `Principal.ID` are always required, including when you set `ApplyPrincipal`.
