...
- The application's object-level authorization: tenant A must not read or write tenant B's objects on the path and verb a case names.
...
`MustDeny` is called from a test. The handler and the Case are both supplied by the test author. The shipped corpus is in-package Go data returned by `Corpus()`. There is no file parser. Treat those fixtures as test data from this repo, not as input from the internet.
...
- Byte-exact also applies to the four default headers. Percent-encoding (`tenant%2DA`), `filename*`, and `http.SetCookie` sanitizing are not decoded. Short needles can match structured values such as a `Set-Cookie` `Expires` date, and can also appear by chance in compressed body bytes (`"7"`, sometimes `"42"`).
...
- `relation_mismatch` fixtures needle the owner id. A same-tenant 403 that names the owner fails those cases. That is a corpus assumption, not a cross-tenant leak.
- `Corpus()` is GET and PUT on `/invoices/inv-a`. Listing, DELETE, and sub-resources are unreached.
